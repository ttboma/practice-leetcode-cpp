...
class Colors:
    ...
    def is_supported(cls) -> bool:
        ...
        return (
            ...
            and platform.system()
            != "Windows"  # Windows cmd doesn't support ANSI by default
        )
...
class DependencyChecker:
    ...
    def check_doxygen(self, required: bool = False) -> bool:
        ...
        try:
            ...
            if required:
                self.logger.error(
                    "Doxygen not found. Cannot build documentation without Doxygen."
                )
                self.logger.error("Please install Doxygen first.")
                return False
            else:
                self.logger.warning(
                    "Doxygen not found. Documentation generation will be skipped."
                )
                system = platform.system()
                if system == "Darwin":
                    ...
                    self.logger.warning(
                        "Install with: apt-get install doxygen (Ubuntu/Debian)"
                    )
                elif system == "Windows":
                    self.logger.warning(
...
class VirtualEnvironmentManager:
    ...
    def create_or_update(self):
        ...
        for package in packages:
            ...
            if not executable.exists():
                ...
                subprocess.run(
                    [str(python_exe), "-m", "pip", "install", package, "-q"], check=True
                )
    ...
    def install_precommit_hooks(self):
        ...
        if precommit_config.exists():
            ...
            precommit_exe = self.detector.get_venv_executable(
                self.venv_dir, "pre-commit"
            )
            ...
            self.logger.success(
                "Pre-commit hooks installed (commit-msg, pre-commit, pre-push)"
            )
...
class CMakeBuildSystem:
    ...
    def create_compile_commands_symlink(self):
        ...
        if compile_commands.exists():
            self.logger.info(
                "Creating compile_commands.json symlink for IDE support..."
            )
            ...
            try:
                ...
                self.logger.success(
                    f"IDE symlink created: compile_commands.json -> {relative_path}"
                )
            except OSError:
                # Fallback to copying on Windows if symlink fails
                shutil.copy2(compile_commands, symlink_path)
                self.logger.success(
                    "IDE file copied: compile_commands.json (symlink not supported)"
                )
...
Examples:
  ...
  python build.py --no-tests               # Build without tests
  ...
  python build.py --open-docs              # Build and open documentation in browser
    ...
    parser.add_argument(
        "-d", "--build-dir", help="Build directory (default: <source>/out/build/<type>)"
    )
    parser.add_argument(
        "-j",
        "--jobs",
        type=int,
        default=PlatformDetector.get_cpu_count(),
    ...
    parser.add_argument(
        ...
        "--run-tests",
        action="store_true",
        help="Run tests after building (overridden by --no-tests)",
    )
    ...
    parser.add_argument(
        "--docs", action="store_true", help="Build documentation (requires Doxygen)"
    )
    parser.add_argument(
        "--open-docs",
        action="store_true",
