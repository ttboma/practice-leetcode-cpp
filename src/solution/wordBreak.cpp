...
bool wordBreakHelperDP(std::string_view sv, const std::vector<std::string>& wordDict) {
    std::vector<bool> dp(sv.size() + 1, false);
    dp[0] = true;
    for (size_t i = 1; i <= sv.size(); ++i) {
        for (const auto& w : wordDict) {
            if (i >= w.size() && dp[i - w.size()] && std::equal(sv.begin() + i - w.size(), sv.begin() + i, w.begin())) {
                dp[i] = true;
                break;
            }
        }
...
bool Solution::wordBreak(std::string s, std::vector<std::string>& wordDict) { return wordBreakHelperDP(s, wordDict); }
